...
- **更新提示**：每次启动检查 GitHub 最新版本（走 HTML 页面重定向，不占用 GitHub API 配额、静默失败、可配置更新源），有新版顶部横幅；点「下载更新」**按当前形态直接下载对应的包（免安装版 / NSIS setup.exe / MSI）到「下载」文件夹**，不跳转 GitHub，**安装版下完即自动运行安装程序并在安装完成后删除安装包**，免安装版则在资源管理器中选中刚下载的文件（详见下文）；点击"忽略此版本"后该版本不再提示
...
- **启动失败可自救**：dsh 起不来时（例如切到了有问题的版本），启动页给出「切换 dsh 版本 / 重试启动 / 打开日志」三个按钮，界面加载不出来也能切回去（见「dsh 版本管理」）
...
2. **按当前形态自动选择安装包**：判据是**运行中 exe 自己的路径**，不是"注册表里有没有卸载项"——所以机器上另装了一份安装版，也不会让免安装版下成安装包。匹配顺序：卸载注册表里**指向该 exe 所在目录**的记录（比对 `InstallLocation`、`DisplayIcon`、卸载命令，兼容尾部反斜杠/大小写/`\\?\` 前缀，环境变量由查询时展开）中带 `WindowsInstaller=1` 或 `msiexec` 卸载命令的 → **MSI 安装版**下载 `.msi`；否则匹配到 NSIS 记录、或 exe 同级有 `uninstall.exe` → **NSIS 安装版**下载 `..._x64-setup.exe`；都没有 → **免安装版**下载 `..._x64-portable.exe`。判据与选择结果都记在 `startup.log` 里（`update download: packaging -> …`、`best match for this build -> …`）。横幅副标题会写明"将下载 xxx（大小）"以及下完是否自动安装。
3. 下载到系统**真正的「下载」文件夹**：Windows 上先取 shell 的 `下载` 已知文件夹（`shell:Downloads`，兼容被重定向/托管的环境），取不到才退回 `%USERPROFILE%\Downloads`。先写 `.part` 再改名，副标题显示"已下载 x / y MB"实时进度，支持 Range 断点续传；`.part` 大小与发布信息不符时丢弃重下；同名文件存在时自动命名为 `xxx (2).exe`，不覆盖旧文件。传输优先走 **node**（OpenSSL + 流式写入 + 进度事件），node 不可用时回退系统 `curl`；http/https 镜像都支持。
4. 下载后校验发布页公布的 `sha256`（不匹配则删除并提示），再 `Unblock-File` 解除"来自互联网"标记。然后分两种形态收尾：
   - **安装版（setup.exe / .msi）**：**直接启动安装程序**（MSI 走 `msiexec /i`，需要管理员权限时由系统弹 UAC；取消 UAC 不会丢失安装包），安装结束后**自动删除已下载的安装包**，横幅显示"安装完成，安装包已自动删除"。安装被取消/失败则保留安装包，横幅给一个「重新安装」按钮就地重跑。
   - **免安装版**：不自动执行（执行只会又开一个实例），改为**在资源管理器中选中刚下载的文件**——用的是系统 `explorer /select,"<完整路径>"`，完整路径原样传递，所以打开的一定是该文件所在的**真实下载目录**。完成提示会带上**完整路径**，即使「下载」被重定向也能一眼看到文件落在哪（日志里同样有）。

> 「打开所在文件夹」按钮可在下载完成后随时重新定位该文件；点「重新下载」可换一个副本，点「更新说明」就地展开 release notes。
> 下载失败（网络、代理、无 node/curl、资产不存在等）会自动回退到打开 GitHub 发布页，按钮不会变成死路；尤其当推导出的资产名 404（发布时改了文件名）时会打开发布页让你手选。
> **安全限制**：只有本次更新检查**公布过的资产地址**会被下载（页面另递地址会被拒绝并改为打开发布页）；自动安装也只接受「下载」文件夹里、文件名确为安装包（`…-setup.exe` / `.msi`）的那份文件。因此横幅无法被页面利用去"抓一个任意程序再执行它"。
...
>
> **启动失败时的自救**：如果切到的版本起不来（服务未就绪、界面加载不出来），启动页会显示原因并给出三个按钮——**切换 dsh 版本**（直接打开上面的版本对话框，切回可用版本或装别的版本）、**重试启动**（不重启应用，就地重新拉起 dsh，适合冷启动超时/端口冲突这类偶发失败）、**打开日志**（打开 `startup.log`）。因此即使界面永远加载不出来也不会被困住，**不需要手改 `dsh-versions.json`**。正常启动后这三个按钮自动隐藏。
...
> 发布时三种产物都会上传。应用内「下载更新」按**当前形态**挑选：以运行中 exe 的路径匹配卸载注册表项（MSI 记录 → 下载 `.msi`；NSIS 记录或同级有卸载器 → 下载 `setup.exe`；都没有 → 下载 `portable.exe`）。判据与选择结果都记在 `startup.log` 里（`update download: packaging -> …`、`best match for this build -> …`）。
>
